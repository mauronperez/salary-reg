...
def test_inference_runs_and_returns_predictions(sample_df):
    """Ensure inference pipeline runs and returns predicted_income column."""
    ...
    assert "predicted_income" in preds_df.columns

    # Predictions should be numeric
    assert pd.api.types.is_numeric_dtype(preds_df["predicted_income"])

    print("✅ Inference pipeline test passed. Predictions:")
    print(preds_df[["predicted_income"]].head())
