...
if TRAIN_FE_PATH.exists():
    ...
    TRAIN_FEATURE_COLUMNS = [c for c in _train_cols.columns if c != "income"]
else:
...
app = FastAPI(title="Income Regression API")

# / → simple landing endpoint to confirm API is alive.
@app.get("/")
def root():
    return {"message": "Income Regression API is running 🚀"}
...
def predict_batch(data: List[dict]):
    ...
    resp = {"predictions": preds_df["predicted_income"].astype(float).tolist()}
    if "actual_income" in preds_df.columns:
        resp["actuals"] = preds_df["actual_income"].astype(float).tolist()
